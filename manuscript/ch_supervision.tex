...
The same loss function applied to explicit B\'{e}zier paths and to implicit neural fields 
produces different geometric outcomes, because gradients propagate differently through each representation. 
...
Shortly to describe, render the SVG, compare it to a target image, update the parameters. 
...
and it works through DiffVG~\cite{liDifferentiableVectorGraphics2020a}.
...
CLIP-based guidance (Section~\ref{sec:clip}) can score how well a rendered SVG matches a text prompt, when no refrence image is exist. 
...
In practice, CLIP guidance is rarely used alone. It typically serves as an early-stage 
alignment signal before diffusion-based supervision stage, 
as in VectorFusion~\cite{jainVectorFusionTexttoSVGAbstracting2022} 
and early SVGDreamer configurations.
...
these methods use a pretrained diffusion model as a perceptual prior, instead of relying on reference images, same as CLIP. 
...
The mechanism is Score Distillation Sampling (SDS), with Variational Score Distillation (VSD) 
introduced by SVGDreamer~\cite{xingSVGDreamerTextGuided2024} to address the oversaturation 
that standard SDS tends to produce. Both are described in Section~\ref{sec:sds_vsd}. 
...
optimization tends to add more primitives to satisfy visual objectives.
Inference cost is also high, every generation requires repeated rendering and 
diffusion evaluation cycles, unlike dataset-trained models that produce output in a single pass.
...
DeepSVG~\cite{carlierDeepSVGHierarchicalGenerative2020}, DeepIcon~\cite{bingDeepIconHierarchicalNetwork2024}, 
and Im2Vec~\cite{reddyIm2VecSynthesizingVector2021} are trained on reconstruction objectives against SVG data, 
and once trained, generation requires no further optimization. LayerTracer~\cite{songLayerTracerCognitiveAlignedLayered2025} 
takes this further: it trains on a synthetically generated serpentine dataset where vector graphics are 
constructed in a deliberate layer order that mimics how designers actually build graphics (Figure~\ref{fig:serpentine}). 
...
and T2V-NPR~\cite{zhangTexttoVectorGenerationNeural2024} all optimize vector parameters from scratch for each input. 
...
SVGFusion~\cite{xingSVGFusionScalableTexttoSVG2025} and T2V-NPR~\cite{zhangTexttoVectorGenerationNeural2024} 
combine both, they learn latent representations from datasets during training, then apply optimization-based at inference. 
...
because the starting point already encodes where objects are. The supervision signal did not change. 
The initialization did, and that made the structural difference. 
...
SVGFusion and T2V-NPR both start optimization from latent embeddings shaped by prior training, 
so the optimizer never begins from random noise. LayerTracer takes the idea further, 
...
This explains why models using the same 
diffusion-based loss produce very different structural outcomes. When meaningful priors are absent, 
diffusion optimization piles on primitives. When structured initialization is present, 
...
but none of the fourteen directly optimizes for it
