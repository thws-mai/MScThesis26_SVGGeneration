...
types: parametric optimization over Bézier primitives 
using differentiable rasterization, transformer-based 
hierarchical sequence models, MLP-parameterized neural 
implicit representations, and latent diffusion 
architectures with vector-specific decoders. 
Chapter~\ref{ch:Landscape} introduced each type, and 
Chapter~\ref{ch:representation} analyzed how 
representational choices within each type determine 
...
constrained domains. LayerTracer and SVGFusion come 
closest to balancing visual quality with structural 
usability among the analyzed set. Chapter~\ref{ch:synthesis} 
summarized these findings in a conditions table and 
identified the specific architectural factors that 
distinguish models on the editability-realism axis.
